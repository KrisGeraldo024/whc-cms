    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="testimonials && testimonials.data" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span> 
                    {{ testimonials.from }} - {{ testimonials.to }} 
                ...
                <div class="flex gap-[16px] items-center">
                    ...
                    <router-link :to="`/testimonials/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Name</th>
                        <th class="px-[24px] py-[8px]">Description</th>
                        <th class="px-[24px] py-[8px]">Position</th>
                ...
                <tbody>
                    <template v-if="testimonials && testimonials.data" v-for="(testimonial, index) in testimonials.data" :key="testimonial.id">
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px]  text-base font-normal">{{ testimonial.order }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ testimonial.name }}</td>     
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ testimonial.description }}</td> 
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ testimonial.position }}</td>                      
                            <td class="px-[24px] py-[8px] ">
                                {{ $moment(testimonial.updated_at ? testimonial.updated_at : testimonial.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black"> <!-- Adjust styling as needed -->
                                    {{ $moment(testimonial.updated_at).format('hh:mm A') }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px]  relative w-[150px]">
                                ...
                                <div
                                    ...
                                    <router-link :to="`/testimonials/${testimonial.id}`"
...
<script setup>
    ...
    onMounted( async () => {
        ...
        pageTitle.setTitle("Testimonials");
        pageTitle.setBreadcrumbs(['Testimonials', 'Testimonials List']);
    ...
    const fetchRecords = async () => {
        try {
            ...
            const testimonials_response = await nuxtApp.$axios.get(`/cms/testimonials?page=${pagination.page}
            &type=${route.params.type}
            &sortBy=${sortBy.value}
            &sortDirection=${sortDirection.value}
            &keyword=${keyword.value}`); 
            // locations.value = location_response.data.map((record) => record.name);
            testimonials.value = testimonials_response.data.records;

            pagination.setTotalPages(testimonials_response.data.records.last_page);
        } catch (error) {
