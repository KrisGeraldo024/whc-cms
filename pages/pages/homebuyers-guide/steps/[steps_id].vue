    ...
    <div class="relative flex justify-start p-[16px] mb-[32px] w-full overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                    ...
                    <div class="bg-[#EAF5FF] rounded-[10px] p-[16px] w-[587px] h-auto flex flex-col gap-[8px]">
                        ...
                        <p class="text-sm text-black font-normal flex">
                            Drag the 
                             ...
                             handle to re-order Buyer’s Guide sections.
                        </p>
                    </div>
                    <button @click="addStep()" type="button" class="p-[16px] border border-ui-color/20 rounded-[10px] flex items-center gap-[4px] text-ui-color text-base font-medium">
                ...
                <Draggable v-if="formData.accordions" v-model="formData.accordions" handle=".handle" item-key="id"  @update:modelValue="onReorder"  class="flex flex-col gap-[16px]">
                    <template #item="{element: accordion, index: accordionIndex}">
                        <div class="cursor-pointer bg-offwhite flex flex-col gap-[16px] rounded-[10px] w-full overflow-hidden" :class="{'h-[58px]' : !isOpen[accordionIndex], 'h-auto' : isOpen[accordionIndex]}">
                            <div @click="isOpen[accordionIndex] = !isOpen[accordionIndex]" class="p-[16px] rounded-t-[10px] flex justify-between items-center" :class="{'bg-offwhite' : !isOpen[accordionIndex], 'bg-[#E8F1FF] border border-ui-color' : isOpen[accordionIndex]}">
                                <div class="flex gap-[10px] text-lg font-semibold" :class="{'text-black' : !isOpen[accordionIndex], 'text-ui-color' : isOpen[accordionIndex]}">
                                    ...
                                    {{`Step ${accordionIndex + 1}`}}
                                </div>
                                <span class="w-[24px] h-[24px]">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none" :class="{'rotate-180': !isOpen[accordionIndex]}">
                            ...
                            <div class="w-full h-auto p-[16px] gap-y-[16px] flex flex-col">
                                <ImageHandler v-if="sectionData"
                                    ...
                                    max="1"
                                    size="5"
                                    :dimension="{width:45, height:43}"
                                    :input_payload="{
                                        identifier: `icon${accordionIndex}`,
                                        id: `icon${accordionIndex}_id`,
                                        category: `icon${accordionIndex}_category`,
                                        category_value: 'icon',
                                        alt: `icon${accordionIndex}_alt`,
                                    }"
                                    :required="true"
                                    :ref="(el) => (imageHandlers[accordionIndex] = el)"
                                    v-model="icons[accordionIndex][0]"
                                    @update:file="handleFileUpdate(`icon${accordionIndex}`, $event)"
                                />
                                <TextField
                                    :key="accordion.id"
                                    v-model="accordion.title"
                                    :name="`accordion_title[${accordionIndex}]`"
                                    :label="'Step Title'"
                                    :placeholder="'Enter title'"
                                    :rules="'required|max:50'"
                                    :hasErrors="hasErrors"
                                />
                                <!-- <TextInput v-model="accordion.title" :name="`accordion_title[${index}]`" :label="`Step Title`" max="50" :placeholder="`Enter title`"/> -->
                                <Quill v-model="accordion.description" :modelValue="accordion.description" :name="`accordion_description[${accordionIndex}]`" :label="`Step Description`" :max="1500" :placeholder="`Start typing...`"/>
                                <input type="hidden" :name="`accordion_id[${accordionIndex}]`" v-model="accordion.id">
                                <!-- <input type="hidden" :name="`accordion_order[${accordionIndex}]`" v-model="accordion.order"> -->
                                <DeleteItem v-if="accordion.id || formData.accordions.length > 1" :id="accordion.id" :model="'Accordion'" :index="accordionIndex" @removeItem="removeStep" class="self-end"/>
                            </div>
                        </div>
                    </template>
                </Draggable>
...
<script setup>
    ...
    onMounted(async () => {
        ...
        formData.accordions.forEach((_, index) => {
            if (!icons.value[index]) {
                icons.value[index] = []; // Initialize each accordion index as an array
            }
        });
        // populateData(sectionData.value);
    });
    const id = route.params.steps_id; 
    ...
    const submit = async () => {
        ...
        icons.value.forEach((file, i) => {
            // accd.icon.forEach((file, index) => {
                form_data.append(`icon${i}[]`, file[0])
            // })
        })
        ...
        formData.accordions.forEach((a, index) => {
            form_data.append(`accordion_description[${index}]`, formData.accordions[index].description)
    ...
    const populateData = (newData) => {
        if (newData) {
            ...
            formData.accordions = newData.accordions?.length ? newData.accordions.map(accordion => ({
                id: accordion.id,
                title: accordion.title,
                description: accordion.description,
                order: accordion.order,
                icon: accordion.images.filter(image => image.category === 'icon')
            })) : [
                { 
                    title: '',
                    description: '',
            ...
            console.log(formData.accordions)
        }
    }
