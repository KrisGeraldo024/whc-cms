    ...
    <div class="relative flex justify-start p-[16px] mb-[32px] w-full overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            ...
            <div v-if="sectionData && sectionData.order !== 2"  class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">{{sectionData.order !== 3 ? 'Icon' : 'Images'}}</h2>
                <ImageHandler 
                    label="Desktop Image" 
                    :data="main_image"
                    max="1"
                    size="5"
                    :dimension="sectionData.order !== 1 ? {width: 413, height: 280} : {width: 1153, height: 490}"
                    :input_payload="{
                ...
                <ImageHandler  v-if="sectionData.order !== 3"
                    label="Mobile Image" 
                    max="1"
                    size="5"
                    :dimension="sectionData.order !== 1 ? {width: 413, height: 280} : {width: 1153, height: 490}"
            ...
            <div v-if="sectionData && sectionData.order !== 1 && sectionData && sectionData.order !== 2" class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
...
<script setup>
    ...
    const id = route.params.buyerSection_id; 
