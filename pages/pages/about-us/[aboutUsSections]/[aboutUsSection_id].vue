    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <TextField
                        v-model="formData.title" name="title" :label="`${formattedTitle} Header Copy`"  placeholder="Enter banner title"
                        :rules="'required|max:60'" 
                        :hasErrors="hasErrors"
                        /> 
                <!-- <TextInput v-model="formData.title" name="title" :label="`${formattedTitle} Header Copy`" max="50" placeholder="Enter banner title"/> -->
                <HowToAdd v-if="sectionData && (sectionData.order === 6 )" :title="formattedTitle" :link="'/awards'"/>
                <Quill v-if="sectionData && (sectionData.order === 2 || sectionData.order === 3 || sectionData.order === 4)" 
                    v-model="formData.description" 
                    :modelValue="formData.description" :name="`description`" 
                    :label="`${formattedTitle} Copy`" :max="1000" :placeholder="`Start typing...`"
                />
            </div>
            <div v-if="sectionData && (sectionData.order === 1 || sectionData.order === 3 || sectionData.order === 4)"  class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <ImageHandler 
                    ...
                    max="1"
                    size="5"
                    :dimension="sectionData.order === 1 ? {width:1280, height:685} : {width:566, height:355}"
                    :input_payload="{
                ...
                <ImageHandler 
                    ...
                    size="5"
                    :data="mobile_image"
                    :dimension="sectionData.order===1 ? {width:375, height:480} : {width:566, height:355}"
                    :input_payload="{
