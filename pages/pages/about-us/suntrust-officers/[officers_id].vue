    ...
    <div class="relative flex justify-start p-[16px] mb-[32px] w-full overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            ...
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <Draggable v-if="formData.boards" v-model="formData.boards" handle=".handle" item-key="id"  @update:modelValue="onReorder"  class="flex flex-col gap-[16px]">
                    <template #item="{element: board, index: boardIndex}">
                        <div class="cursor-pointer bg-offwhite flex flex-col gap-[16px] rounded-[10px] w-full overflow-hidden" 
                        :class="{'h-[58px]' : !isOpen[boardIndex], 'h-auto' : isOpen[boardIndex]}">
                            <div  @click="isOpen[boardIndex] = !isOpen[boardIndex]" class="p-[16px] rounded-t-[10px] flex justify-between items-center" 
                            :class="{'bg-offwhite' : !isOpen[boardIndex], 'bg-[#E8F1FF] border border-ui-color' : isOpen[boardIndex]}">
                                <div class="flex gap-[10px] text-lg font-semibold" :class="{'text-black' : !isOpen[boardIndex], 'text-ui-color' : isOpen[boardIndex]}">
                                    ...
                                    {{`Officer ${boardIndex < 9 ? '0' : ''}${boardIndex + 1}`}}
                                </div>
                                <span class="w-[24px] h-[24px]">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none" :class="{'rotate-180': !isOpen[boardIndex]}">
                            ...
                            <div class="w-full h-auto p-[16px] gap-y-[16px] flex flex-col">
                                <ImageHandler v-if="sectionData"
                                    label="Officer Image" 
                                    :data="board.main_image"
                                    max="1"
                                    size="5"
                                    :dimension="{width: 273, height: 273}"
                                    :input_payload="{
                                        identifier: `officer_image${boardIndex}`,
                                        id: `officer_image${boardIndex}_id`,
                                        category: `officer_image${boardIndex}_category`,
                                        category_value: 'main_image',
                                        alt: `officer_image${boardIndex}_alt`,
                                    }"
                                    :required="true"
                                    :ref="(el) => (imageHandlers[boardIndex] = el)"
                                    v-model="main_image[boardIndex][0]"
                                    @update:file="handleFileUpdate(`officer_image${boardIndex}`, $event)"
                                />
                                <div class="flex justify-between gap-[16px]">
                                    <TextField class="w-full"
                                    v-model="board.name" :name="`board_name[${boardIndex}]`" :label="`Officer Name`" :placeholder="`Enter Name`"
                                    :rules="'required|max:70'" :hasErrors="hasErrors"
                                    /> 
                                    <TextField class="w-full"
                                    v-model="board.position" :name="`board_position[${boardIndex}]`" :label="`Position Title`" :placeholder="`Enter Position`"
                                    :rules="'required|max:70'" :hasErrors="hasErrors"
                                    /> 
                                </div>
                                <input type="hidden" :name="`board_id[${boardIndex}]`" v-model="board.id">
                                <DeleteItem v-if="board.id || formData.boards.length > 1" :id="board.id" :model="'Board'" :index="boardIndex" @removeItem="removeBoard" class="self-end"/>
                            </div>
                        </div>
                    </template>
                </Draggable>
...
<script setup>
    ...
    onMounted(async () => {
        ...
        formData.boards.forEach((_, index) => {
            if (!main_image.value[index]) {
                main_image.value[index] = []; // Initialize each accordion index as an array
            }
        });
    });
    const id = route.params.officers_id; 
    ...
    const handleFileUpdate = (field, file) => {
        ...
        if (!main_image.value[idx]) {
            main_image.value[idx] = []; 
        }

        // Update the formData at the specific index and field name
        main_image.value[idx]= file; 
    };
    ...
    const submit = async () => {
        ...
        main_image.value.forEach((file, i) => {
            // accd.officer_image.forEach((file, index) => {
                form_data.append(`officer_image${i}[]`, file[0])
            // })
        })
        // formData.mobile_image.forEach((file, index) => {
        //     form_data.append(`mobile_image[]`, file)
        // })
        formData.boards.forEach((a, index) => {
            form_data.append(`board_order[${index}]`, index + 1)
        })
        form_data.append('has_button', 0)
    ...
    const onReorder = async (newOrder) => {
        ...
        try {
            const response = await nuxtApp.$axios.post(`/cms/re-order`, {
                ...
                fetchRecords();
                nuxtApp.$toast.success('Officers reordered successfully!');
            }); 
    ...
    const populateData = (newData) => {
        if (newData) {
            ...
            formData.boards = newData.boards?.length ? newData.boards.map(board => ({
                id: board.id,
                name: board.name,
                position: board.position,
                order: board.order,
                main_image: board.images.filter(image => image.category === 'main_image')
            })) : [
                { 
                    name: '',
                    position: '',
            ...
            // formData.has_button = newData.has_button
    ...
    const addOfficer = () => {
        // if (formData.boards.length < 6) {
            formData.boards.push({
                name: '',
                position: '',
                order: formData.boards.length + 1,
                logo: [],
                id: '',
                main_image: []
            })
    ...
    const removeBoard = (index) => {
        formData.boards.splice(index, 1);
        fetchRecords();
    }
