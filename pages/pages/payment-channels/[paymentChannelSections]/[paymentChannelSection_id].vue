<template>
    <div class="relative flex justify-start p-[16px] mb-[32px] w-full overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            ...
            <div v-if="sectionData && sectionData.name === 'Get a Quote Banner'"  class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">{{`${sectionData.name } Image`}}</h2>
                <ImageHandler 
                    label="Desktop Image" 
                    :data="main_image"
                    max="1"
                    size="5"
                    :dimension="{width: 632, height: 301}"
                    :input_payload="{
            ...
            <!-- <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <div v-if="formData.has_button" v-for="(button, index) in formData.buttons" :key="index" class="">
                    <Button v-if="formData.has_button" :name="`button`" :button="button"/>
                </div>
            </div>  -->
...
<script setup>
    ...
    const id = route.params.paymentChannelSection_id; 
