<template>
    <div class="relative flex justify-start p-[16px] mb-[32px] w-full overflow-y-auto">
        <Form id="form" v-if="sectionData && formattedTitle" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
...
<script setup>
    ...
    // const { data: sectionData, error, status: isLoading } = useAsyncData(`page-section-${id}`, async () => {
    //     try {
    //         // Use Axios to make the GET request
    //         const res = await nuxtApp.$axios.get(`/cms/page-section/${id}`);
