    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <Quill v-if="sectionData.order === 1" v-model="formData.description" :name="`description`" :label="`Description`" :max="400" :placeholder="`Start typing...`"/>
                <!-- <TextInput v-model="formData.title" name="title" :label="`${formattedTitle} Header Copy`" max="50" placeholder="Enter banner title"/> -->
                <!-- <HowToAdd v-if="sectionData && (sectionData.name === 'Featured House & Lots' || sectionData.name === 'Featured Condominiums')" :title="formattedTitle" link="/properties"/> -->
            </div>
            <div v-if="sectionData && (sectionData.order === 1 || sectionData.order === 2)"  class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">{{`${formattedTitle } Image`}}</h2>
                <ImageHandler 
                    :label="sectionData.order === 1 ? 'Banner Image' : 'Icon'" 
                    :data="main_image"
                    max="1"
                    size="5"
                    :dimension="sectionData.order === 1 ? {width: 153, height: 456} : {width: 79, height: 79}"
                    :input_payload="{
            ...
            <!-- <div v-if="sectionData && sectionData.order === 2"  class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">{{`${formattedTitle } Brochure`}}</h2>
                <ImageHandler 
                    label="Brochure File" 
                    :data="pdf"
                    max="1"
            ...
            <div v-if="sectionData && sectionData.order === 2" class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <div v-if="formData.has_button" v-for="(button, index) in formData.buttons" :key="index" class="">
                    <Button v-if="formData.has_button" :name="`button`" :button="button" :show-link="false"/>
                </div>
            </div>
...
<script setup>
    ...
    const id = route.params.sellersGuideSection_id; 
    ...
    const submit = async () => {
        ...
        // formData.pdf.forEach((file, index) => {
    ...
    const populateData = (newData) => {
        if (newData) {
            ...
            // pdf.value = newData.files?.filter(image => image.category === 'pdf') || [];
        }
    }
