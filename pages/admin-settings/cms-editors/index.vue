    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="users && users.data" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span> 
                    {{ users.total > 0 ? users.from : 0 }} {{ users.total > 0 ? ' - ' +  users.to : '' }} 
                ...
                <div class="flex gap-[16px] items-center">
                    ...
                    <router-link  :to="`/admin-settings/cms-editors/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Editor Name</th>
                        <th class="px-[24px] py-[8px]">Role</th>
                        <th class="px-[24px] py-[8px]">Email</th>
                ...
                <tbody>
                    <template v-if="users && users.data" v-for="(user, index) in users.data" :key="user.id">
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ user.user_detail.full_name }}</td>
                            <td class="px-[24px] py-[8px] text-base font-normal flex gap-[8px] items-center h-[64px]" :class="{'text-purple' : user.role.name === 'Admin', 'text-ui-color' : user.role.name !== 'Admin'}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="7" height="8" viewBox="0 0 7 8" 
                                :class="{'fill-purple' : user.role.name === 'Admin', 'fill-ui-color' : user.role.name !== 'Admin'}">
                                <circle cx="3.5" cy="4" r="3.5" />
                                </svg>
                                {{ user.role.name }}
                            </td>
                            <td class="px-[24px] py-[8px] text-base font-normal text-ui-colr">
                                <a :href="`mailto:${user.email}`">{{ user.email }}</a>
                            </td>
                           
                            <td class="px-[24px] py-[8px] ">
                                {{ $moment(user.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black"> <!-- Adjust styling as needed -->
                                    {{ $moment(user.updated_at).format('hh:mm A') }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px]  relative w-[150px]">
                                ...
                                <div
                                    ...
                                    <router-link :to="`/admin-settings/cms-editors/${user.id}/update`"
                                    ...
                                    <button v-if="user.role.name !== 'Admin'"
...
<script setup>
    ...
    onMounted( () => {
        ...
        pagination.reset();
        pageTitle.setTitle(`CMS Editors`);
        pageTitle.setBreadcrumbs(['Admin Settings', 'CMS Editors']);
    ...
    const fetchRecords = async () => {
        try {
            ...
            const user_response = await nuxtApp.$axios.get(`/cms/users?page=${pagination.page}
            &sortBy=${sortBy.value}
            &sortDirection=${sortDirection.value}
            &keyword=${keyword.value}`); 
            // locations.value = location_response.data.map((record) => record.name);
            users.value = user_response.data.records;
            pagination.setTotalPages(user_response.data.records.last_page);
        } catch (error) {
