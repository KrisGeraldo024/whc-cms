<template>
    <div class="relative w-full p-[16px] bg-offwhite flex flex-col gap-[16px] overflow-y-auto mb-[32px]">
        <Form v-if="unitData" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <div class="grid grid-cols-2 gap-[16px]">
                    ...
                    :rules="'required'"
                    optionalMessage="Please paste the Google Maps embed link here."
                    :hasErrors="hasErrors" />
                </div>
                <div class="grid grid-cols-3 gap-[16px]" v-if="propertyType === 'house-and-lots'">
                    <SelectField v-if="unit_types" 
                    label="Unit Type"
                    name="unit_type_id"
                    v-model="formData.unit_type"
                    placeholder="Choose a Unit Type"
            ...
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                v-model="formData.floor_plan_title"
            ...
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                v-model="formData.unit_gallery_title"
                        ...
                        <div class="cursor-pointer bg-offwhite flex flex-col gap-[16px] rounded-[10px] w-full overflow-hidden" 
                            ...
                            :class="{'bg-offwhite' : !isOpen[featureIndex], 'bg-[#E8F1FF] border border-ui-color' : isOpen[featureIndex]}">
                                <div class="flex gap-[10px] text-lg font-semibold" :class="{'text-black' : !isOpen[featureIndex], 'text-ui-color' : isOpen[featureIndex]}">
                                    ...
                                    {{`Room Features ${featureIndex < 9 ? '0' : ''}${featureIndex + 1}`}}
                                </div>
                                <span class="w-[24px] h-[24px]">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none" 
                                    :class="{'rotate-180': !isOpen[featureIndex]}">
                            ...
                            <div class="w-full h-auto p-[16px] gap-y-[16px] flex flex-col">
                                ...
                                optionalMessage="Max 100 characters" 
                                :hasErrors="hasErrors"/>
                                <Quill v-if="isClient" v-model="formData.features[featureIndex].description" 
                                :modelValue="formData.features[featureIndex].description" :name="`feature_description[${featureIndex}]`" 
                                :label="`Feature description`" :max="600" :placeholder="`Enter your description here...`"/>
            ...
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                :name="`see_more_title`"
                v-model="formData.see_more_title"
...
import DraggableComponent from '~/components/Draggable.vue';
    ...
    const floor_plan_image = ref(null);
    const thumbnail = ref(null);
    const unit_gallery = ref(null);
    const see_more_image = ref(null);
    const  feature_icon = ref(null);
    ...
    onMounted(async () => {
        ...
        pageTitle.setTitle(`Edit Unit`);
        pageTitle.setBreadcrumbs(['Properties', property_type, await propertyData.value.name, 'Units', await unitData.value.name, 'Edit Unit']);

        pageTitle.setPageFrom('Units List');
        pageTitle.setPageFromRoute('/properties/' + propertyType + '/' + route.params.id + '/units');
        isClient.value = true;
    });
    ...
    const fetchRecords = async () => {
        ...
        try {
            ...
            const unit_response = await nuxtApp.$axios.get(`/cms/units/${id}`); 
            unitData.value = unit_response.data.record;

            populateData(unitData.value);
        } catch (error) {
            console.error('Error:', error);
        }
    };

    const populateData = (unitData) => {
        formData.title = unitData.name;
        formData.starts_at = unitData.starts_at;
        formData.address = unitData.location;
        formData.gmap_url = unitData.gmap_url ? unitData.gmap_url : '';
        formData.unit_type = unitData.unit_type.id;
        formData.lot_area = unitData.lot_area;
        formData.floor_area = unitData.floor_area;

        formData.bedrooms = unitData.bedroom;
        formData.t_and_b = unitData.t_and_b;
        formData.storeys = unitData.storeys;
        formData.powder_rooms = unitData.powder_room;

        formData.floor_plan_title = unitData.page_sections.find(section => section.name === 'Floor Plan')?.title || '';
        formData.yt_title = unitData.videos?.title || '';
        formData.yt_url = unitData.videos?.yt_url || '';
        formData.unit_gallery_title = unitData.page_sections.find(section => section.name === 'Unit Gallery')?.title || '';
        formData.see_more_title = unitData.page_sections.find(section => section.name === 'See more Units Banner')?.title || '';

        main_image.value = unitData.images.filter(image => image.category === 'main_image');
        floor_plan_image.value = unitData.page_sections.find(section => section.name === 'Floor Plan')?.images || [];
        thumbnail.value = unitData.images.filter(image => image.category === 'thumbnail');
        unit_gallery.value = unitData.page_sections.find(section => section.name === 'Unit Gallery')?.images || [];
        see_more_image.value = unitData.page_sections.find(section => section.name === 'See more Units Banner')?.images || [];
        feature_icon.value = unitData.features?.map(feature => feature.images) || [];


        formData.meta_title = unitData.metadata?.meta_title || '';
        formData.meta_description = unitData.metadata?.meta_description || '';
        formData.link_rel = unitData.metadata?.link_rel || '';
        formData.tracking_code = unitData.tracking_code;

        formData.enabled =  Number(unitData.enabled);
        formData.features = unitData.features;
    ...
    const submit = async () => {
        ...
        if (isValid) {
            ...
            try {
                const response = await nuxtApp.$axios.post(`/cms/units/${id}`, form_data , {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                }).then((response) => {
                    ...
                    nuxtApp.$toast.success('Unit updated successfully!');
                }); 
