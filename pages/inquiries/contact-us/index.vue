                    ...
                    <table class="w-full min-w-max">
                        ...
                        <thead class="">
                            <tr class=" bg-white text-black/60 text-left">
                                ...
                                <th class="opacity-60 font-[500] pr-[24px] py-[8px]">Contact Number</th>
                                ...
                                <th class="opacity-60 font-[500] pr-[24px] py-[8px]">Type of Inquiry</th>
                        ...
                        <tbody v-if="records && records.data" v-for="(record, index) in records.data">
                            ...
                            <tr class="h-[40px] bg-white text-black font-normal text-base text-left ">
                                ...
                                <td class="pr-[24px] py-[8px]">{{ record.type }}</td>
...
onMounted(() => {
    ...
    pageTitle.setBreadcrumbs(['Inquiries', 'Contact Us']);
...
const fetchRecords = async () => {
    try {
        ...
        const records_response = await nuxtApp.$axios.get(`/cms/inquiries?inquiry_type=Contact Us
...
const exportToCSV = async () => {
    const records_response = await nuxtApp.$axios.get(`/cms/export-inquiries?inquiry_type=Contact Us
    &startDate=${filterValues.startDate}
    &endDate=${filterValues.endDate}`);
...
const generateExportFilename = (type) => {
    ...
    const filename = `${contentType}_Contact-Us_${exportType}_.${type === 'csv' ? 'csv' : 'xlsx'}`;
    return filename;
};
