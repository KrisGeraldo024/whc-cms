...
onMounted(() => {
    ...
    pageTitle.setTitle(`In-House Sales Group`);
    pageTitle.setBreadcrumbs(['Inquiries', 'In-House Sales Group']);
...
const fetchRecords = async () => {
    try {
        ...
        const records_response = await nuxtApp.$axios.get(`/cms/inquiries?inquiry_type=In House Sales Group
...
const exportToCSV = async () => {
    const records_response = await nuxtApp.$axios.get(`/cms/export-inquiries?inquiry_type=In House Sales Group
    &startDate=${filterValues.startDate}
    &endDate=${filterValues.endDate}`);
...
const generateExportFilename = (type) => {
    ...
    const filename = `${contentType}_In-House-Sales-Group_${exportType}_.${type === 'csv' ? 'csv' : 'xlsx'}`;
    return filename;
};
