    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            ...
            <div class="flex pb-[3rem]" v-else>
                ...
                <!-- Table 2 -->
                <div ref="horizScrollContainer" class="overflow-x-scroll scroll-custom" @mousedown="startDragging"
                    ...
                    <table class="w-max">
                        ...
                        <thead>
                            <tr class=" bg-white text-black/60 text-left">
                                ...
                                <th class="opacity-60 font-[500] pr-[24px] py-[8px] rounded-tr-[10px]">Property Type
                                </th>
                                <th class="opacity-60 font-[500] pr-[24px] py-[8px]">Community</th>
                                <th class="opacity-60 font-[500] pr-[24px] py-[8px]">Unit Type</th>
                                <th class="opacity-60 font-[500] pr-[24px] py-[8px]">Remarks</th>
                            </tr>
                        </thead>
                        <!-- Contents  -->
                        <tbody v-if="records && records.data" v-for="(record, index) in records.data">
                            ...
                            <tr class="h-[40px] bg-white text-black font-normal text-base text-left ">
                                ...
                                <td class="pr-[24px] py-[8px]">{{ record.property_type }}</td>
                                <td class="pr-[24px] py-[8px]">{{ record.property_name }}</td>
                                <td class="pr-[24px] py-[8px]">{{ record.unit_type }}</td>
...
onMounted(() => {
    ...
    pageTitle.setTitle(`Get a Quote`);
    pageTitle.setBreadcrumbs(['Inquiries', 'Get a Quote']);
...
const fetchRecords = async () => {
    try {
        ...
        const records_response = await nuxtApp.$axios.get(`/cms/inquiries?inquiry_type=Get a Quote
...
const exportToCSV = async () => {
    const records_response = await nuxtApp.$axios.get(`/cms/export-inquiries?inquiry_type=Get a Quote
    &startDate=${filterValues.startDate}
    &endDate=${filterValues.endDate}`);
    ...
    const csvData = records.map(record => ({
        ...
        'Community': record.property_name,
...
const generateExportFilename = (type) => {
    ...
    const filename = `${contentType}_Get-Quote_${exportType}_.${type === 'csv' ? 'csv' : 'xlsx'}`;
    return filename;
};
