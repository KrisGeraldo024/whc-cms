    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        <div class="relative w-full flex p-[16px] justify-between items-center bg-white">
            <form @submit="onSubmit" class="gap-y-[8px] w-full">
                <label for="search" class="text-base font-medium text-black/70">
                    Search for Award name or Awarding body
                </label>
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="awards && awards.data" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span>
                    {{ awards.from }} - {{ awards.to }}
                ...
                <div class="flex gap-[16px] items-center">
                    ...
                    <router-link :to="`/awards/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table
                class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        <th class="px-[24px] py-[8px] rounded-[10px] cursor-pointer flex gap-[8px]"
                            @click="toggleSort('order')"> Seq.
                        ...
                        <th class="px-[24px] py-[8px]">Award Name</th>
                        ...
                        <th class="px-[24px] py-[8px]">Awarding Body</th>
                        <th class="px-[24px] py-[8px] flex gap-[8px] cursor-pointer" @click="toggleSort('updated_at')">
                            Date Edited
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18"
                                fill="none">
                ...
                <tbody>
                    <template v-if="awards && awards.data" v-for="(award, index) in awards.data" :key="award.id">
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px]  text-base font-normal">{{ award.order }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ award.name }}</td>
                            <!-- <td class="px-[24px] py-[8px] text-base font-normal">{{ award.awarding_body }}</td> -->

                            <!-- <td class="px-[24px] py-[8px] ">
                                {{ $moment(award.date ? award.date : award.date).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black"> 
                                    {{ $moment(award.date).format('hh:mm A') }}
                                </span>
                            </td> -->
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ award.awarding_body }}</td>
                            <!-- <td class="px-[24px] py-[8px] flex gap-[8px] items-center h-[64px]" :class="{'text-success' : award.enabled, 'text-orange' : !award.enabled}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="7" height="8" viewBox="0 0 7 8" 
                                :class="{'fill-success' : award.enabled, 'fill-orange' : !award.enabled}">
                                <circle cx="3.5" cy="4" r="3.5" />
                                </svg>
                                {{ award.enabled ? 'Yes' : 'No' }}
                            </td> -->
                            <td class="px-[24px] py-[8px] ">
                                {{ $moment(award.updated_at ? award.updated_at : award.updated_at).format('MMM DD,YYYY') }}<br>
                                <span class="text-sm text-black"> <!-- Adjust styling as needed -->
                                    {{ $moment(award.updated_at).format('hh:mm A') }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px]  relative w-[150px]">
                                <button @click.stop="toggleMenu(index)">
                                    <svg width="40" height="20" viewBox="0 0 40 20" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <rect width="40" height="20" rx="10" fill="#E9E9E9" />
                                        <circle cx="10" cy="10" r="2" fill="#2A2A2A" />
                                        <circle cx="20" cy="10" r="2" fill="#2A2A2A" />
                                        <circle cx="30" cy="10" r="2" fill="#2A2A2A" />
                                    </svg>
                                </button>
                                <div v-if="activeMenuIndex === index"
                                    ...
                                    <router-link :to="`/awards/${award.id}`"
                                    ...
                                    <button class="flex items-center gap-[10px] text-[#F12222]" @click="
                                        openDeletePopup(
                                            `/cms/awards/${award.id}`
                                        )
                                        ">
...
onMounted(async () => {
    ...
    pageTitle.setTitle("Awards");
    pageTitle.setBreadcrumbs(['Awards', 'Awards List']);
...
const fetchRecords = async () => {
    try {
        ...
        const awards_response = await nuxtApp.$axios.get(`/cms/awards?page=${pagination.page}
            &type=${route.params.type}
            &sortBy=${sortBy.value}
            &sortDirection=${sortDirection.value}
            &keyword=${keyword.value}`);
        // locations.value = location_response.data.map((record) => record.name);
        awards.value = awards_response.data.records;

        pagination.setTotalPages(awards_response.data.records.last_page);
    } catch (error) {
