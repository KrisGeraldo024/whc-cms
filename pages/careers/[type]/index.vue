    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        <div class="relative w-full flex p-[16px] justify-between items-center bg-white">
            <form @submit="onSubmit" class="gap-y-[8px] w-full">
                <label for="search" class="text-base font-medium text-black/70">
                    Search for a Job Title, location, or employment type
                </label>
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="careers && careers.data" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span> 
                    {{ careers.from }} - {{ careers.to }} 
                ...
                <div class="flex gap-[16px] items-center">
                    ...
                    <router-link :to="`/careers/${route.params.type}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
                    ...
                    <span class="w-[21px] h-[21px]">
                        ...
                        Add New {{ formattedTitle.slice(0, formattedTitle.length - 1) }}
                    </router-link>
                </div>
            </div>
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Job Title</th>
                        <th class="px-[24px] py-[8px]" v-if="route.params.type == 'job-listings'">Location</th>
                        <th class="px-[24px] py-[8px]" v-if="route.params.type == 'job-listings'">Employment Type</th>
                        ...
                        <th class="px-[24px] py-[8px]">Active</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    <template v-if="careers && careers.data" v-for="(career, index) in careers.data" :key="career.id">
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px]  text-base font-normal">{{ career.order }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ career.title }}</td>
                            <td class="px-[24px] py-[8px] text-base font-normal" v-if="route.params.type == 'job-listings'">{{ career.location.name }}</td>
                            <td class="px-[24px] py-[8px] text-base font-normal" v-if="route.params.type == 'job-listings'">{{ career.employment_type.name }}</td>
                           
                            <td class="px-[24px] py-[8px] ">
                                {{ $moment(career.date ? career.date : career.date).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black"> <!-- Adjust styling as needed -->
                                    {{ $moment(career.date).format('hh:mm A') }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px]  relative w-[150px]">
                                ...
                                <div
                                    ...
                                    <router-link :to="`/careers/${route.params.type}/${career.id}`"
...
<script setup>
    ...
    onMounted( async () => {
        ...
        pageTitle.setTitle(formattedTitle.value);
        pageTitle.setBreadcrumbs(['Careers', formattedTitle.value]);
    ...
    const fetchRecords = async () => {
        try {
            ...
            const jobs_response = await nuxtApp.$axios.get(`/cms/careers?page=${pagination.page}
            &type=${route.params.type}
            &sortBy=${sortBy.value}
            &sortDirection=${sortDirection.value}
            &keyword=${keyword.value}`); 
            // locations.value = location_response.data.map((record) => record.name);
            careers.value = jobs_response.data.records;

            pagination.setTotalPages(jobs_response.data.records.last_page);
        } catch (error) {
