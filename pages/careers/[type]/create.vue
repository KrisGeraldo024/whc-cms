<template>
    ...
    onMounted(() => {
        ...
        pageTitle.setTitle(`Add ${formattedTitle.value.slice(0, formattedTitle.value.length - 1)}`);
        pageTitle.setBreadcrumbs(['Careers', formattedTitle.value, 'Add Career']);
    ...
    const submit = async () => {
            ...
            try {
                const response = await nuxtApp.$axios.post(`/cms/careers`, form_data , {
                    ...
                    useRouter().push(`/careers/${route.params.type}/${record.id}`);
                }); 
