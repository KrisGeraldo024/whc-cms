    ...
    <div class="p-4 overflow-y-scroll relative">
        <Form
            ...
            <div
                ...
                <Draggable v-if="formData.accordions && formData.accordions.length > 0" v-model="formData.accordions" handle=".handle" item-key="id"  @update:modelValue="onReorder"  class="flex flex-col gap-[16px]">
                    <template #item="{element: accordion, index: accordionIndex}">
                        <div class="cursor-pointer bg-offwhite flex flex-col gap-[16px] rounded-[10px] w-full overflow-hidden"
                            :class="{
                                'h-[58px]': !isOpen[accordionIndex],
                                'h-auto': isOpen[accordionIndex],
                            }">
                            <div
                                @click="isOpen[accordionIndex] = !isOpen[accordionIndex]"
                                class="p-[16px] rounded-t-[10px] flex justify-between items-center"
                                :class="{
                                    'bg-offwhite': !isOpen[accordionIndex],
                                    'bg-[#E8F1FF] border border-ui-color':
                                        isOpen[accordionIndex],
                                }">
                                <div
                                    class="flex gap-[10px] text-lg font-semibold"
                                    :class="{
                                        'text-black': !isOpen[accordionIndex],
                                        'text-ui-color': isOpen[accordionIndex],
                                    }">
                                ...
                                <span class="w-[24px] h-[24px]">
                                    <svg
                                        ...
                                        viewBox="0 0 24 25"
                                        fill="none"
                                        :class="{ 'rotate-180': !isOpen[accordionIndex] }">
                                        <path
                                            fill-rule="evenodd"
                                            clip-rule="evenodd"
                            ...
                            <div
                                ...
                                <form-fieldsQuill
                                    ...
                                    :name="`accordion_description[${accordionIndex}]`"
                                    :label="`Step Description`"
                                    :max="400"
                                    :placeholder="`Start typing...`" />
                                <input
                                    type="hidden"
                                    :name="`accordion_id[${accordionIndex}]`"
            ...
            <div class="bg-white p-4 flex flex-col gap-4 rounded-lg">
                ...
                <div class="grid grid-cols-2 gap-[16px]">
                    <Field
                        ...
                        name="button_name">
                        <TextField
                            :rules="formData.isPublished ? 'required|max:50' : ''"
                            v-bind="field"
                            v-model="formData.button_name"
                    ...
                    <div class="relative">
                        <Field
                            ...
                            name="link">
                            <TextField
                                :rules="formData.isPublished ? 'required' : ''"
                                v-bind="field"
...
const paymentPlatform = ref(null);

const fetchRecords = async () => {
    formData.accordions = [];
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/payment-methods/${route.params.methodId}/payment-platforms/${route.params.platformId}`
        ); // Add your endpoint
        ...
        paymentPlatform.value = response.data.record;
        populateData(paymentPlatform.value);
    } catch (error) {
...
onMounted(() => {
    ...
    pageTitle.setBreadcrumbs([
        'Payment Channels',
        'Payment Methods',
        'Edit Payment Platform',
    ]);

    pageTitle.setPageFrom('Payment Platforms');
...
const onSubmit = async values => {
    try {
        ...
        form_data.append('isPublished', values.isPublished);
...
const populateData = newData => {
    ...
    if (newData) {
        ...
        formData.accordions = newData.accordions
        formData.isPublished = newData.buttonActive == 1 || newData.buttonActive == 'true' ? true : false
        formData.button_name = newData.buttonText
        formData.link = newData.buttonLink
        isLinkOut.value = newData.buttonIsLinkOut == 1 || newData.buttonIsLinkOut == '1' ? true : false
        formData.sequence = newData.sequence
    }
};
