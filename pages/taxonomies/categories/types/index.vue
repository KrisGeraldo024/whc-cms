<template>
    <div class="overflow-y-scroll">
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.type_count || "pending"}}</span>
                        </td>
                        <td class="py-[34.5px]">
                            <span>
                                {{
                        ...
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.type_count === 0"
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Type' : 'Add a Type'">
            <Form
                @submit="onSubmit"
                :initial-values="formInitialValues">
                <TextField
                    label="Type Name"
                    v-model="formInitialValues.typeName"
                    name="typeName"
...
onMounted(() => {
    ...
    pageTitle.reset();
    pageTitle.setTitle('Types');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Tile-Categories', 'Types']);
    fetchRecords();
});
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/type?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        ...
        nuxtApp.$toast.error('Failed to load type');
    }
};
//  here
...
const formInitialValues = computed(() => ({
    typeName: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Type updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Type added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.errors.name[0] ||
                'An error occurred while saving the type'
        );
    } finally {
