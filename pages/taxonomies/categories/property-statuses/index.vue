<template>
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                <div>
                    ...
                    {{ totalRecords }} Property Statuses
                </div>
                <!-- this -->
                <button
                    @click="openPopup()"
                    ...
                    <span> Add a Property Status</span>
                </button>
            </div>
            <!-- Table  -->
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.property_status_count}}</span>
                        </td>
                        <!-- this  -->
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.property_status_count === 0"
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Status' : 'Add a Status'">
            <Form
                @submit="onSubmit"
                :initial-values="formInitialValues">
                <TextField
                    label="Status Name"
                    v-model="formInitialValues.statusName"
                    name="statusName"
...
onMounted(() => {
    ...
    pageTitle.reset();
    pageTitle.setTitle('Property Statuses');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Properties', 'Property Statuses']);
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/property-statuses?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        console.log('doing');
...
const formInitialValues = computed(() => ({
    statusName: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Project status updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Project status added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.errors.name[0] ||
                'An error occurred while saving the Project status'
        );
    } finally {
