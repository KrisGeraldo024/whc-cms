        ...
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                <div>Displaying <b>{{ records?.length }}</b> of
                    <b>{{ totalRecords }}</b> application(s)</div>
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.application_count || "pending"}}</span>
                        </td>
                        <td class="py-[34.5px]">
                            <span>
                                {{
                        ...
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.application_count === 0"
        ...
        <PopupForm
            ...
            <Form
                ...
                <TextField
                    ...
                    v-model="formInitialValues.applicationName"
...
onMounted(() => {
    ...
    pageTitle.reset();
    pageTitle.setTitle('Applications');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Tile-Categories', 'Applications']);
    fetchRecords();
});
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/application?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        ...
        nuxtApp.$toast.error('Failed to load application');
    }
};
//  here
...
const formInitialValues = computed(() => ({
    applicationName: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Application updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Application added successfully!');
        }
