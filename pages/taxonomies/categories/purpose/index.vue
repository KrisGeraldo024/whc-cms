<template>
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                <div>Displaying <b>{{ records?.length }}</b> of
                    <b>{{ totalRecords }}</b> purpose(s)</div>
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.purpose_count || "pending"}}</span>
                        </td>
                        <td class="py-[34.5px]">
                            <span>
                                {{
                        ...
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.purpose_count === 0"
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Purpose' : 'Add a Purpose'">
            <Form
                @submit="onSubmit"
                :initial-values="formInitialValues">
                <TextField
                    label="Purpose Name"
                    v-model="formInitialValues.purposeName"
                    name="purposeName"
                    placeholder="Enter a purpose name"
...
onMounted(() => {
    ...
    pageTitle.reset();
    pageTitle.setTitle('Purpose');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Tile-Categories', 'Purpose']);
    fetchRecords();
});
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/purpose?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        ...
        nuxtApp.$toast.error('Failed to load purpose');
    }
};
//  here
...
const formInitialValues = computed(() => ({
    purposeName: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Purpose updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Purpose added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.errors.name[0] ||
                'An error occurred while saving the purpose'
        );
    } finally {
