<template>
    <div class="overflow-y-scroll">
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.finish_count || "pending"}}</span>
                        </td>
                        <td class="py-[34.5px]">
                            <span>
                                {{
                        ...
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.finish_count === 0"
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Finish' : 'Add a Finish'">
            <Form
                @submit="onSubmit"
                :initial-values="formInitialValues">
                <TextField
                    label="Finish Name"
                    v-model="formInitialValues.finishName"
                    name="finishName"
                    placeholder="Enter a finish name"
                ...
                <div class="flex gap-[24px] mt-[16px] ml-auto max-w-max">
                    ...
                    <button
                        ...
                        {{ isEditing ? 'Save Changes' : 'Add Finish' }}
                    </button>
                </div>
            </Form>
        </PopupForm>
...
onMounted(() => {
    ...
    pageTitle.reset();
    pageTitle.setTitle('Finish');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Tile-Categories', 'Finish']);
    fetchRecords();
});
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/finish?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        ...
        nuxtApp.$toast.error('Failed to load finish');
    }
};
//  here
...
const formInitialValues = computed(() => ({
    finishName: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Finish updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Finish added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.errors.name[0] ||
                'An error occurred while saving the finish'
        );
    } finally {
