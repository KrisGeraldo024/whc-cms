    ...
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                <div>
                    Displaying <b>{{ records.length }}</b> of
                    <b>{{ totalRecords }}</b> material(s)
                </div>
                <!-- this -->
                <button
                    @click="openPopup()"
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.properties_count}}</span>
                        </td>
                        <!-- this  -->
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.properties_count === 0"
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Material' : 'Add a Material'">
            <Form
                id="form"
                @submit="onSubmit"
                ...
                <TextField
                    label="Material Name"
                    v-model="formInitialValues.locationName"
                    name="locationName"
                ...
                <div class="flex flex-col gap-[8px] mt-[8px]">
                    ...
                    :data="formInitialValues.main_image"
                    max="1"
                    size="5"
                    :input_payload="{
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/property-locations?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        // console.log('doing');
...
const formInitialValues = computed(() => ({
    main_image: editingRecord.value?.images.filter(image => image.category === 'main_image') || [],
    locationName: editingRecord.value?.name || '',
}));
...
const handleFileUpdate = (field, file) => {
    main_image.value = file; // Updates the specific image field
};
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (main_image.value && main_image.value.length > 0) {
            form_data.append('main_image[]', main_image.value[0]);
        }
        form_data.append('name', values.locationName);
        form_data.append('type', 'property_location');
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Material updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', form_data,  {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
            nuxtApp.$toast.success('Material added successfully!');
        }
