<template>
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                <div>Displaying <b>{{ records?.length }}</b> of
                    <b>{{ totalRecords }}</b> Feature(s)</div>
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.feature_count || "pending"}}</span>
                        </td>
                        <td class="py-[34.5px]">
                            <span>
                                {{
                        ...
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.feature_count === 0"
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Feature' : 'Add a Feature'">
            <Form
                @submit="onSubmit"
                :initial-values="formInitialValues">
                <TextField
                    label="Feature Name"
                    v-model="formInitialValues.featureName"
                    name="featureName"
...
onMounted(() => {
    ...
    pageTitle.reset();
    pageTitle.setTitle('Features');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Tile-Categories', 'Features']);
    fetchRecords();
});
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/feature?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        ...
        nuxtApp.$toast.error('Failed to load feature');
    }
};
//  here
...
const formInitialValues = computed(() => ({
    featureName: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Feature updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Feature added successfully!');
        }
