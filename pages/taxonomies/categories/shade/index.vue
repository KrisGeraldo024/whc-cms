<template>
    <div class="overflow-y-scroll">
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.shade_count || "pending"}}</span>
                        </td>
                        <td class="py-[34.5px]">
                            <span>
                                {{
                        ...
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.shade_count === 0"
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Shade' : 'Add a Shade'">
            <Form
                @submit="onSubmit"
                :initial-values="formInitialValues">
                <TextField
                    label="Shade Name"
                    v-model="formInitialValues.shadeName"
                    name="shadeName"
                    placeholder="Enter a shade name"
                ...
                <div class="flex gap-[24px] mt-[16px] ml-auto max-w-max">
                    ...
                    <button
                        ...
                        {{ isEditing ? 'Save Changes' : 'Add Shade' }}
                    </button>
                </div>
            </Form>
        </PopupForm>
...
onMounted(() => {
    ...
    pageTitle.reset();
    pageTitle.setTitle('Shades');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Tile-Categories', 'Shades']);
    fetchRecords();
});
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/shade?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        ...
        nuxtApp.$toast.error('Failed to load shade');
    }
};
//  here
...
const formInitialValues = computed(() => ({
    shadeName: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Shade updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Shade added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.errors.name[0] ||
                'An error occurred while saving the shade'
        );
    } finally {
